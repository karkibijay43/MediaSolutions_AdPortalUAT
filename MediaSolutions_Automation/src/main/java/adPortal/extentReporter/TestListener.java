...
public class TestListener  implements ITestListener {
	 ...
	 * " failed..."); log.info((result.getMethod().getMethodName() + " failed!"));
	 * 
	 ...
	 * String timeStamp = Util.getCurrentTimeStamp(); // get timestamp String
	 * testMethodName = result.getName().toString().trim(); String screenShotName =
	 * testMethodName + timeStamp + ".png"; String fileSeperator =
	 * System.getProperty("file.separator"); String reportsPath =
	 * System.getProperty("user.dir") + fileSeperator + "TestReport" + fileSeperator
	 * + "screenshots"; log.info("Screen shots reports path - " + reportsPath); try
	 * { File file = new File(reportsPath + fileSeperator + testClassName); // Set
	 * // screenshots // folder if (!file.exists()) { if (file.mkdirs()) {
	 * log.info("Directory: " + file.getAbsolutePath() + " is created!"); } else {
	 * log.info("Failed to create directory: " + file.getAbsolutePath()); }
	 * 
	 * }
	 * 
	 * File screenshotFile = ((TakesScreenshot)
	 * driver).getScreenshotAs(OutputType.FILE); targetLocation = reportsPath +
	 * fileSeperator + testClassName + fileSeperator + screenShotName;// define //
	 * location File targetFile = new File(targetLocation);
